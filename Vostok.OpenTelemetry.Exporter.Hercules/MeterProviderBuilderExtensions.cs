...
public static class MeterProviderBuilderExtensions
{
    ...
    public static MeterProviderBuilder AddHerculesExporter(
        this MeterProviderBuilder builder,
        Action<HerculesMetricExporterOptions>? configureExporter) =>
        builder.AddHerculesExporter(null, configureExporter);

    public static MeterProviderBuilder AddHerculesExporter(
        this MeterProviderBuilder builder,
        Action<HerculesMetricExporterOptions>? configureExporter,
        Action<MetricReaderOptions>? configureReader) =>
        ...
        Action<MetricReaderOptions>? configureReader)
    {
        ...
        {
            var exporterOptionsMonitor = serviceProvider.GetRequiredService<IOptionsMonitor<HerculesMetricExporterOptions>>();
            ...
            return BuildExporter(sink, () => exporterOptionsMonitor.Get(name), readerOptions);
        });

        return builder;
    }

    private static PeriodicExportingMetricReader BuildExporter(
        IHerculesSink sink,
        Func<HerculesMetricExporterOptions> exporterOptionsProvider,
